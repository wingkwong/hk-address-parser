    ...
    return list(
        map(
            (
                lambda record: (
                    {
                        "chi": eliminate_lang_keys(
                            record["Address"]["PremisesAddress"]["ChiPremisesAddress"]
                        ),
                        "eng": eliminate_lang_keys(
                            record["Address"]["PremisesAddress"]["EngPremisesAddress"]
                        ),
                        "geo": record["Address"]["PremisesAddress"][
                            "GeospatialInformation"
                        ],
                        "score": record["ValidationInformation"]["Score"],
                    }
                )
            ),
...
def parse_address(address, normalized_ocgio_result):
    for record in normalized_ocgio_result:
        """
        20200121 wingkwong: deprecated matches as OGCIO now provides ValidationInformation Score
        """
...
#     switcher = {
#         constant.OGCIO_KEY_STREET: lambda: search_occurance_for_street(
#             address, ogcio_record_element
#         ),
#         constant.OGCIO_KEY_VILLAGE: lambda: search_occurance_for_village(
#             address, ogcio_record_element
#         ),
#         constant.OGCIO_KEY_BLOCK: lambda: search_occurance_for_block(
#             address, ogcio_record_element
#         ),
#         constant.OGCIO_KEY_PHASE: lambda: search_occurance_for_phase(
#             address, ogcio_record_element
#         ),
#         constant.OGCIO_KEY_ESTATE: lambda: search_occurance_for_estate(
#             address, ogcio_record_element
#         ),
#         constant.OGCIO_KEY_REGION: lambda: search_occurance_for_region(
#             address, ogcio_record_element
#         ),
#         constant.OGCIO_KEY_BUILDING_NAME: lambda: search_occurance_for_building_name(
#             address, ogcio_record_element
#         ),
#     }

#     return switcher.get(ogcio_record_elementKey, None)()
...
#     return search_similarity_for_street_or_village(
#         constant.OGCIO_KEY_STREET,
#         address,
#         address_to_be_searched,
#         building_no_from,
#         building_no_to,
#     )
...
#             [block_no, block_descriptor],
#         )

#         if block_no:
#             if not try_to_match_any_number(address, int(block_no)):
...
#         if phase_no:
#             if not try_to_match_any_number(address, int(phase_no)):
#                 match.confident = constant.CONFIDENT_MULTIPLIER_NAME_ONLY
#             return match
...
#         return Match(
#             constant.CONFIDENT_ALL_MATCH, constant.OGCIO_KEY_ESTATE, [estate_name]
#         )
#     return None
...
#         return Match(constant.CONFIDENT_ALL_MATCH, constant.OGCIO_KEY_ESTATE, [region])
#     return None
...
#         return Match(
#             constant.CONFIDENT_ALL_MATCH, constant.OGCIO_KEY_ESTATE, [building_name]
#         )
#     else:
#         match_percentage, match_word = find_partial_match(address, building_name)
#         if match_percentage > 0:
#             match = Match(
#                 constant.CONFIDENT_ALL_MATCH, constant.OGCIO_KEY_ESTATE, [match_word]
#             )
#             match.confident = modify_confident_by_partial_match_percentage(
#                 match.confident, match_percentage
#             )
#             return match
#         return None

# def search_similarity_for_street_or_village(
#     type, address, address_to_search, building_no_from, building_no_to
# ):
#     sim = Match(0, type, [])
...
#         if match_percentage > 0:
#             sim.confident = modify_confident_by_partial_match_percentage(
#                 constant.CONFIDENT_ALL_MATCH, match_percentage
#             )
...
#     match = {"match_percentage": 0, "matched_word": None}
...
#         * match_percentage
#         * match_percentage
#         * constant.CONFIDENT_MULTIPLIER_PARTIAL_MATCH
#     )


# def find_maximum_non_overlapping_matches(address, matches):
#     if len(matches) == 1:
#         if matches[0].matched_words is not None and match_all_matched_words(
#             address, matches[0].matched_words
#         ):
#             return matches
#         return []
...
#             local_longest_match = find_maximum_non_overlapping_matches(
#                 sub_address,
#                 [
#                     _match
#                     for _match in matches
#                     if _match.matched_key != match.matched_key
#                 ],
#             )
...
#     address_includes_word = map(
#         lambda matched_words, address: matched_words in address, matched_words, address
#     )

#     return reduce(lambda p, c: (p and c), [address_includes_word])
...
#     def __repr__(self):
#         return "Match({}, {}, {})".format(
#             self.confident, self.matched_key, self.matched_words
#         )
